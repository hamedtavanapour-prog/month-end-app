...
function printTable(title,subtitle,headers,rows){
  ...
  const cellHtml=cell=>cell&&typeof cell==='object'&&!Array.isArray(cell)&&'value' in cell?`${safe(cell.value)}<small>${safe(cell.detail||'')}</small>`:safe(cell);
  const tb=rows.map(row=>`<tr>${row.map(cell=>`<td>${cellHtml(cell)}</td>`).join('')}</tr>`).join('');
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${safe(title)}</title><style>@page{size:landscape;margin:14mm;}*{box-sizing:border-box;}body{margin:0;color:#17202a;font-family:Arial,sans-serif;font-size:10px;}h1{margin:0 0 5px;font-size:20px;}p{margin:0 0 16px;color:#59636e;font-size:9px;line-height:1.45;}table{width:100%;border-collapse:collapse;}thead{display:table-header-group;}tr{break-inside:avoid;}th{background:#26313d;color:#fff;padding:7px 6px;text-align:left;font-size:9px;}td{padding:6px;border-bottom:1px solid #dfe3e7;}td small{display:block;margin-top:2px;color:#65717d;font-size:6px;font-style:italic;line-height:1.3;}tr:nth-child(even) td{background:#f6f7f8;}td:nth-child(4),td:nth-child(6),td:nth-child(7),td:nth-child(8),th:nth-child(4),th:nth-child(6),th:nth-child(7),th:nth-child(8){text-align:right;}@media print{body{print-color-adjust:exact;-webkit-print-color-adjust:exact;}}</style></head><body><h1>${safe(title)}</h1><p>${safe(subtitle)}</p><table><thead>${th}</thead><tbody>${tb}</tbody></table><script>window.onload=function(){window.print();}<\/script></body></html>`);
  w.document.close();
}
