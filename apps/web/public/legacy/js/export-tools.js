...
function printTable(title,subtitle,headers,rows){const w=window.open('','_blank','width=1000,height=750');const th=`<tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr>`;const tb=rows.map(r=>`<tr>${r.map(c=>`<td>${c??''}</td>`).join('')}</tr>`).join('');w.document.write(`<!DOCTYPE html><html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;padding:20px;font-size:12px;}h2{margin:0 0 4px;}p{margin:0 0 14px;color:#666;}table{width:100%;border-collapse:collapse;}th{background:#333;color:#fff;padding:7px;text-align:left;}td{padding:6px 7px;border-bottom:1px solid #ddd;}tr:nth-child(even) td{background:#f9f9f9;}</style></head><body><h2>${title}</h2><p>${subtitle}</p><table><thead>${th}</thead><tbody>${tb}</tbody></table><script>window.onload=function(){window.print();}<\/script></body></html>`);w.document.close();}
