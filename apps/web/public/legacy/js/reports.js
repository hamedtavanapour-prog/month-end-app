...
function switchRepTab(tab){['usage','value','rorders'].forEach(t=>{document.getElementById('rtab-'+t).classList.toggle('active',t===tab);document.getElementById('rep-'+t).style.display=t===tab?'block':'none';});if(tab==='value'){populateValueDates();renderValueReport();}if(tab==='rorders')renderOrdersReport();if(tab==='usage')renderReport();}
