...
function normalizeMenuLibraryItem(item,index=0){
  ...
  const normalized={
    ...
    productKnowledge:String(item?.productKnowledge||'').trim(),
    cookTime:String(item?.cookTime||'').trim(),
    cookingInstructions:String(item?.cookingInstructions||'').trim(),
    platingInstructions:String(item?.platingInstructions||'').trim(),
    presentation:String(item?.presentation||'').trim(),
    accompaniment:String(item?.accompaniment||'').trim(),
    equipment:String(item?.equipment||'').trim(),
    kitchenNotes:String(item?.kitchenNotes||'').trim(),
    contentAccessKeys:item?.contentAccessKeys&&typeof item.contentAccessKeys==='object'&&!Array.isArray(item.contentAccessKeys)?{...item.contentAccessKeys}:{},
    sourcePages:item?.sourcePages&&typeof item.sourcePages==='object'&&!Array.isArray(item.sourcePages)?{...item.sourcePages}:{},
    variants,
...
function ensureMenuLibrary(){
  ...
  if(ensureKitchenDinnerMenu())changed=true;
  return changed;
}

function ensureKitchenDinnerMenu(){
  const kitchenVersion=Number(state.kitchenMenuVersion)||0;
  if(kitchenVersion>=3)return false;
  if(!Array.isArray(globalThis.KITCHEN_MENU_MAY_2026))return false;
  let menu=state.menus.find(entry=>entry.id==='menu-kitchen-dinner-may-2026');
  if(!menu){menu={id:'menu-kitchen-dinner-may-2026',departmentId:'kitchen',departmentIds:['kitchen'],name:'Kitchen Dinner Menu',description:'Dinner product knowledge, cooking, plating, and presentation guides',active:true,archived:false,sourceFile:'',importedAt:'',items:[]};state.menus.push(menu);}
  const previousItems=Array.isArray(menu.items)?menu.items:[];
  menu.items=globalThis.KITCHEN_MENU_MAY_2026.map((source,index)=>{
    const previous=previousItems.find(item=>prepItemKey(item.name)===prepItemKey(source.name));
    const preserved=kitchenVersion>=1&&previous?{...previous,imageUrl:source.imageUrl,contentAccessKeys:source.contentAccessKeys,sourcePages:source.sourcePages,source:source.source}:source;
    return normalizeMenuLibraryItem({...preserved,id:previous?.id||`kitchen-may-2026-${prepItemKey(source.name).replace(/\s+/g,'-')}`,price:previous?.price||source.price||''},index);
  });
  menu.departmentId='kitchen';menu.departmentIds=['kitchen'];menu.active=true;menu.archived=false;
  menu.description='Dinner product knowledge, cooking, plating, and presentation guides';
  menu.sourceFile='COOK AND PRESENT - ENGLISH - May 2026.pdf + Product Knowledge - Dinner - CAN - March 2026.pdf';
  menu.importedAt=new Date().toISOString();state.kitchenMenuVersion=3;
  return true;
}
...
function renderMenuItemEditor(){
  const item=menuItemEditDraft;const menu=getSettingsMenu(menuItemEditingMenuId);const body=document.getElementById('menu-item-view-body');if(!item||!menu||!body)return;
  const kitchenFields=menuDepartmentIds(menu).includes('kitchen')?`<div class="kitchen-editor-sections"><div class="menu-item-editor-section-head"><div><span class="label">Kitchen knowledge sections</span><p>Each section stays separate so access can be restricted independently later.</p></div></div><label><span>Product knowledge / overview</span><textarea oninput="updateMenuItemDraftField('productKnowledge',this.value)">${escapeHtml(item.productKnowledge)}</textarea></label><label><span>Cook time</span><input type="text" value="${escapeHtml(item.cookTime)}" oninput="updateMenuItemDraftField('cookTime',this.value)"></label><label><span>Cooking instructions</span><textarea oninput="updateMenuItemDraftField('cookingInstructions',this.value)">${escapeHtml(item.cookingInstructions)}</textarea></label><label><span>Plating instructions</span><textarea oninput="updateMenuItemDraftField('platingInstructions',this.value)">${escapeHtml(item.platingInstructions)}</textarea></label><label><span>Presentation</span><textarea oninput="updateMenuItemDraftField('presentation',this.value)">${escapeHtml(item.presentation)}</textarea></label><label><span>Accompaniment</span><textarea oninput="updateMenuItemDraftField('accompaniment',this.value)">${escapeHtml(item.accompaniment)}</textarea></label><label><span>Equipment / flatware</span><textarea oninput="updateMenuItemDraftField('equipment',this.value)">${escapeHtml(item.equipment)}</textarea></label><label><span>Kitchen notes</span><textarea oninput="updateMenuItemDraftField('kitchenNotes',this.value)">${escapeHtml(item.kitchenNotes)}</textarea></label><label><span>Image URL</span><input type="text" value="${escapeHtml(item.imageUrl)}" oninput="updateMenuItemDraftField('imageUrl',this.value)"></label></div>`:'';
  body.innerHTML=`<div class="product-view-head"><div><span class="detail-eyebrow">${escapeHtml(menu.name)}</span><h3 id="menu-item-view-title">${menuItemEditIsNew?'Add Menu Item':`Edit ${escapeHtml(item.name)}`}</h3></div><button class="detail-close" type="button" aria-label="Close menu item editor" onclick="cancelMenuItemEditor()">&times;</button></div><div class="menu-item-edit-fields"><label><span>Item name</span><input type="text" value="${escapeHtml(item.name)}" oninput="updateMenuItemDraftField('name',this.value)"></label><label><span>Section</span><input type="text" value="${escapeHtml(item.category)}" oninput="updateMenuItemDraftField('category',this.value)"></label><label><span>Price</span><input type="text" inputmode="decimal" value="${escapeHtml(item.price)}" oninput="updateMenuItemDraftField('price',this.value)"></label></div>${menuItemVariantEditorHtml(item)}${menuItemIngredientEditorHtml(item)}${kitchenFields}<div class="menu-item-service-fields"><label><span>Method</span><input list="menu-method-options" value="${escapeHtml(item.method)}" placeholder="Shake &amp; Strain" oninput="updateMenuItemDraftField('method',this.value)"><datalist id="menu-method-options"><option value="Shake & Strain"><option value="Shake & Double Strain"><option value="Shake & Dump"><option value="Shake & Top"><option value="Build in Glass"><option value="Stir & Strain"><option value="Blended"></datalist></label><label><span>Garnish</span><input type="text" value="${escapeHtml(item.garnish)}" placeholder="Dehydrated lime wheel" oninput="updateMenuItemDraftField('garnish',this.value)"></label><label class="wide"><span>General description</span><textarea oninput="updateMenuItemDraftField('description',this.value)">${escapeHtml(item.description)}</textarea></label></div><div class="modal-actions">${menuItemEditIsNew?'':`<button class="btn btn-ghost-danger" type="button" onclick="deleteCurrentMenuItem()">Delete Item</button>`}<button class="btn btn-secondary" type="button" onclick="cancelMenuItemEditor()">Cancel</button><button class="btn btn-primary" type="button" onclick="saveMenuItemEditor()">Save Item</button></div>`;
}

function updateMenuItemDraftField(field,value){if(!menuItemEditDraft||!['name','category','price','description','method','garnish','imageUrl','productKnowledge','cookTime','cookingInstructions','platingInstructions','presentation','accompaniment','equipment','kitchenNotes'].includes(field))return;menuItemEditDraft[field]=String(value||'');}
...
function openMenuItemImage(url,name){
  let lightbox=document.getElementById('menu-item-image-lightbox');
  if(!lightbox){lightbox=document.createElement('div');lightbox.id='menu-item-image-lightbox';lightbox.className='menu-item-image-lightbox';lightbox.setAttribute('role','dialog');lightbox.setAttribute('aria-modal','true');lightbox.addEventListener('click',event=>{if(event.target===lightbox)closeMenuItemImage();});document.body.appendChild(lightbox);}
  lightbox.innerHTML=`<button class="menu-item-image-lightbox-close" type="button" aria-label="Close full-size image" onclick="closeMenuItemImage()">&times;</button><img src="${escapeHtml(url)}" alt="${escapeHtml(name)}"><span>${escapeHtml(name)}</span>`;
  lightbox.classList.add('active');document.body.classList.add('menu-image-lightbox-open');
  lightbox.querySelector('button')?.focus();
}
function closeMenuItemImage(){document.getElementById('menu-item-image-lightbox')?.classList.remove('active');document.body.classList.remove('menu-image-lightbox-open');}
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.getElementById('menu-item-image-lightbox')?.classList.contains('active'))closeMenuItemImage();});
function openMenuItemView(id,preserveVariant=false){
  ...
  const section=(label,value,key)=>value?`<section class="product-view-section kitchen-knowledge-section" data-content-access="${escapeHtml(item.contentAccessKeys?.[key]||'')}"><div class="label">${escapeHtml(label)}</div><p class="menu-method-copy">${escapeHtml(value)}</p></section>`:'';
  const kitchenView=menuDepartmentIds(menu).includes('kitchen')?`${section('Product knowledge',item.productKnowledge,'productKnowledge')}${item.cookTime?`<div class="kitchen-cook-time"><span>Cook time</span><strong>${escapeHtml(item.cookTime)}</strong></div>`:''}${section('Cooking instructions',item.cookingInstructions,'cookingInstructions')}${section('Plating instructions',item.platingInstructions,'platingInstructions')}${section('Presentation',item.presentation,'presentation')}${section('Accompaniment',item.accompaniment,'accompaniment')}${section('Equipment & flatware',item.equipment,'equipment')}${section('Kitchen notes',item.kitchenNotes,'kitchenNotes')}`:`${item.description?section('How to make it',item.description,'description'):''}`;
  const sourcePages=[item.sourcePages?.cookAndPresent?`Cook & Present pp. ${item.sourcePages.cookAndPresent}`:'',item.sourcePages?.productKnowledge?`Product Knowledge p. ${item.sourcePages.productKnowledge}`:''].filter(Boolean).join(' · ');
  const imageUrlEncoded=encodeURIComponent(item.imageUrl||''),imageNameEncoded=encodeURIComponent(item.name||'');
  const imageView=item.imageUrl?`<button class="menu-item-hero-image" type="button" data-content-access="${escapeHtml(item.contentAccessKeys?.image||'')}" aria-label="View full-size image of ${escapeHtml(item.name)}" onclick="openMenuItemImage(decodeURIComponent('${imageUrlEncoded}'),decodeURIComponent('${imageNameEncoded}'))"><img src="${escapeHtml(item.imageUrl)}" alt="${escapeHtml(item.name)}"><span class="menu-item-image-magnifier" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="6"></circle><path d="m16 16 4 4M11 8v6M8 11h6"></path></svg><small>View full size</small></span></button>`:'';
  body.innerHTML=`<div class="product-view-head"><div><span class="detail-eyebrow">${escapeHtml(menu.name)}</span><h3 id="menu-item-view-title">${escapeHtml(item.name)}</h3><div class="product-view-meta"><span class="sub-badge">${escapeHtml(item.category||'Uncategorized')}</span>${item.price?`<span class="sub-badge">$${escapeHtml(item.price)}</span>`:''}${item.method?`<span class="sub-badge">${escapeHtml(item.method)}</span>`:''}</div></div><div class="detail-heading-actions"><button class="btn btn-secondary btn-sm" type="button" onclick="editMenuItemRecipe('${menu.id}','${item.id}')">Edit Item</button><button class="detail-close" type="button" aria-label="Close menu item" onclick="closeModal('modal-menu-item-view')">&times;</button></div></div><div class="menu-item-detail-layout ${imageView?'has-image':''}">${imageView}<div class="menu-item-detail-copy">${variants.length>1?`<div class="menu-size-switch" role="group" aria-label="Recipe size">${variants.map(variant=>`<button class="${variant.id===activeVariant.id?'active':''}" type="button" onclick="setMenuItemViewVariant('${item.id}','${variant.id}')">${escapeHtml(variant.name)}</button>`).join('')}</div>`:''}<div class="product-view-section" data-content-access="${escapeHtml(item.contentAccessKeys?.ingredients||'')}"><div class="label">Ingredients${variants.length>1?` - ${escapeHtml(activeVariant.name)}`:''}</div><div class="menu-recipe-list">${ingredients.length?ingredients.map(entry=>{const encoded=encodeURIComponent(entry.name);return`<div class="menu-recipe-row"><div class="menu-recipe-main">${entry.amount?`<span class="menu-recipe-amount">${escapeHtml(entry.amount)}</span>`:''}${entry.product?`<button class="menu-recipe-link" type="button" onclick="openRecipeProduct('${entry.product.id}','${menu.id}','${item.id}')">${escapeHtml(entry.name)}</button>`:`<button class="menu-recipe-link" type="button" onclick="openRecipePrep(decodeURIComponent('${encoded}'),'${menu.id}','${item.id}')">${escapeHtml(entry.name)}</button>`}</div></div>`;}).join(''):'<p class="menu-method-copy">No ingredients have been added yet.</p>'}</div></div>${kitchenView}<div class="menu-service-summary">${activeVariant.glassware?`<div><span>Glassware</span><strong>${escapeHtml(activeVariant.glassware)}</strong></div>`:''}${item.garnish?`<div><span>Garnish</span><strong>${escapeHtml(item.garnish)}</strong></div>`:''}</div>${sourcePages?`<p class="kitchen-source-pages">${escapeHtml(sourcePages)}</p>`:''}</div></div><div class="modal-actions"><button class="btn btn-secondary" type="button" onclick="closeModal('modal-menu-item-view')">Close</button></div>`;
